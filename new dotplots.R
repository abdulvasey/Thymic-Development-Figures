...
library(tidyverse)
library(cowplot)
#srt<- readRDS("/Users/avymohd/Documents/Single Cell Analysis/Seurats from Ben/allsamps-new3000.1000.1000-100000.3MAD10.SCT.regressed.int.rds")
#DefaultAssay(srt)<-"RNA"
#srt[['integrated']] <- NULL
#srt[['SCT']] <- NULL
#saveRDS(srt,"/Users/avymohd/Documents/Single Cell Analysis/Seurats from Ben/allsamps_rna_only.rds")
srt<- readRDS("/Users/avymohd/Documents/Single Cell Analysis/Seurats from Ben/allsamps_rna_only.rds")
meta <-  read_csv("/Users/avymohd/Documents/Single Cell Analysis/figure scripts for paper/figure1/final meta/finalmeta_allversions.csv")
...
#srt <- RenameIdents(srt, `mTECII/III/IV T` = "mTEC 2 T")
#srt <- RenameIdents(srt, `mTEC 2 T` = "mTECII T")
#srt <- RenameIdents(srt, `mTEC 3/4 T` = "mTECIII/IV T")
#srt <- RenameIdents(srt, `mTECneuro T` = "TECneuro T")
#srt <- RenameIdents(srt, `mTECmyo T` = "TECmyo T")
...
#DimPlot(srt, reduction = "tsne",pt.size = 0.3)+NoAxes()
...
DotPlot(sub, features = feature)+ scale_color_viridis_c(option = "rocket", direction = -1) + theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, face = "bold", size = 10, hjust=1,vjust=0.5),axis.text.y = element_text(face = "bold", size = 10))
dp<-DotPlot(sub, features = feature)+ scale_color_viridis_c(option = "rocket", direction = -1) + theme_minimal() +
...
dp[["plot_env"]][["object"]]<-NULL
saveRDS(dp,file="/Users/avymohd/Documents/Single Cell Analysis/figure scripts for paper/figure1/dpepi.RDS")
#ggsave("/Users/avymohd/Documents/Single Cell Analysis/figure scripts for paper/figure1/dpepi.jpg",h=4.5,w=7,dpi=400)
#marks<-FindMarkers(srt,ident.1 = "Basal E")

####other cell types
#srt <- RenameIdents(srt, `DP-Thymocyte T` = "SP/DP Thymocyte")
#srt <- RenameIdents(srt, `DN-Thymocyte T` = "DN Thymocyte")
...
#srt <- RenameIdents(srt, `Dentritic T` = "Dendritic T")
#idents=c("DN Thymocyte","SP/DP Thymocyte","T Cell BLE","B Cell BL","Macrophage TBLE","Dendritic T","Myoepithelial B","Fibroblast 1 T",'Fibroblast-1 cycling T',"Fibroblast 2 T","VSMC TBLE","Smooth Muscle BLE","Cartilage B","Endothelial TBLE","Lymphatic Endo. TL","Neuronal TBLE","Neuroendocrine BL")
...
DotPlot(sub2, features = genes)+ scale_color_viridis_c(option = "rocket", direction = -1) + theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, face = "bold", size = 10, hjust=1,vjust=0.5),axis.title.y = element_blank(),axis.text.y = element_text(face = "bold", size = 10))
dp<-DotPlot(sub2, features = genes)+ scale_color_viridis_c(option = "rocket", direction = -1) + theme_minimal() +
  theme(axis.title.x=element_blank(),axis.title.y=element_blank(),axis.text.x = element_text(angle = 90, size = 12, hjust=1,vjust=0.5),axis.text.y = element_text(size = 12))
#dp + scale_y_discrete(labels=idents)
dp[["plot_env"]][["object"]]<-NULL
saveRDS(dp,file="/Users/avymohd/Documents/Single Cell Analysis/figure scripts for paper/figure1/dpother.RDS")
#ggsave("/Users/avymohd/Documents/Single Cell Analysis/figure scripts for paper/figure1/OTHERepi.jpg",h=4.5,w=7,dpi=400)


#ggsave("/Users/avymohd/Documents/Single Cell Analysis/figure scripts for paper/figure1/comb.jpg",dpi=400,w=7,h=8)